...
class GroupController(KesslerController):
    ...
    def __init__(self):
        ...
        # THESE ARE THE BEST VALUES FROM MY GA: [-439, -266, -137, -264, -156, -36, -196, -53, 126, 53, 172, 310, 113, 353, 463]
        # Declare fuzzy sets for the ship thrust (HHow hard or long it will turn on the thrusters)
        ship_thrust['BackwardStrong'] = fuzz.trimf(ship_thrust.universe, [-439, -266, -137]) # Backwards strong
        ship_thrust['BackwardWeak'] = fuzz.trimf(ship_thrust.universe, [-264, -156, -36]) # Backwards Weak
        ship_thrust['Neutral'] = fuzz.trimf(ship_thrust.universe, [-196, -53, 126]) # Neutral (Don't move)
        ship_thrust['ForwardWeak'] = fuzz.trimf(ship_thrust.universe, [53, 172, 310]) # Forward Weak
        ship_thrust['ForwardStrong'] = fuzz.trimf(ship_thrust.universe, [113, 153, 463]) # Forward Strong

        # THESE ARE WHAT I ORIGINALLY HAD SET
        # # Declare fuzzy sets for the ship thrust (HHow hard or long it will turn on the thrusters)
        # ship_thrust['BackwardStrong'] = fuzz.trimf(ship_thrust.universe, [-480, -480, -300]) # Backwards strong
        # ship_thrust['BackwardWeak'] = fuzz.trimf(ship_thrust.universe, [-300, -100, 0]) # Backwards Weak
        # ship_thrust['Neutral'] = fuzz.trimf(ship_thrust.universe, [-150, 0, 150]) # Neutral (Don't move)
        # ship_thrust['ForwardWeak'] = fuzz.trimf(ship_thrust.universe, [0, 100, 300]) # Forward Weak
        # ship_thrust['ForwardStrong'] = fuzz.trimf(ship_thrust.universe, [300, 480, 480]) # Forward Strong
